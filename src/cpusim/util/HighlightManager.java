    ...
    private void highlightText(RAM ram, int[] addresses)
    {
        for (int address : addresses) {
            ...
            if (sourceLine != null) {
                ...
                if (!file.canRead()) {
                    Dialogs.showErrorDialog(desktop.getStage(),
                            "CPU Sim could not find the file to open and highlight:  " +
                                    file.getAbsolutePath(),
                            "IO Error", "Debug Mode");
                    return;
                }
                Tab newTabForFile = desktop.newTabForFile(file);
                EditorPaneController epc = desktop.getTabEditorContollers().get(newTabForFile);
                TextArea text = epc.getTextArea();
