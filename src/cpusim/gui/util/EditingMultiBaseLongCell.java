...
public class EditingMultiBaseLongCell<T> extends TableCell<T, Long> {
    ...
    public EditingMultiBaseLongCell(Base base) {
        this.base = base;
        ...
        if (base.getBase().equals("Bin")) {
        	...
        	if( s.length()%8 != 0){
        		for( int i = s.length()%8; i < 8 ; i ++){
        			System.out.println(" No style: adding a zero" );
        			s = "0" + s;
        		}
        	}
        	return Convert.fromAsciiStringToLong(s, cellSize + numZerosAdded);
        }
        else { // base is type Decimal
        	System.out.println("convertString: " + base.getBase() );
    ...
    private void setTooltipsAndCellSize() {
    	...
    	if (o instanceof Register) {
            ...
            cellSize = ((RAMLocation) o).getRam().getCellSize();
            if (((RAMLocation) o).getSourceLine() != null) {
            	tooltipStringProperty.set((String.valueOf(((RAMLocation) o).getSourceLine().getLine())
