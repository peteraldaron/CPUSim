...
import cpusim.util.Assert.AssertionFailureException;
            ...
            textField.focusedProperty().addListener(new ChangeListener<Boolean>(){
                ...
                public void changed(ObservableValue<? extends Boolean> arg0, 
                    Boolean arg1, Boolean arg2) {
                        if (!arg2) {
                            try{
                                int newInt = (int) Convert.fromAnyBaseStringToLong(textField.getText());
                                if (newInt >= 0){
                                    try{
                                        commitEdit(newInt);
                                    ...
                                    if (textField.getScene() != null){
                                        Dialogs.showErrorDialog(
                                            (Stage)textField.getScene().getWindow(), 
                                            "This column requires non-negative integer values",
                                            "Error", "CPU Sim");
                                        cancelEdit();
                                    }
                                }
                                ...
                                if (textField.getScene() != null){
                                    Dialogs.showErrorDialog(
                                        (Stage)textField.getScene().getWindow(), 
                                        "This column requires positive integer values", 
                                        "Error", "CPU Sim");
                                    cancelEdit();
                                }
                            }
