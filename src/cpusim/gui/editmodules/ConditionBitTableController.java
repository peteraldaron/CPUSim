...
public class ConditionBitTableController extends ModuleController implements Initializable {
    ...
    private void fixClonesToUseCloneRegisters()
    {
        for (int i = 0; i < clones.length; i++) {
            ...
            Assert.That(cloneRegister != null, "No clone register of " +
...
//    }
    ...
    public void checkValidity(ObservableList modules)
    {
        ...
        ConditionBit[] conditionBits = new ConditionBit[modules.size()];

        for (int i = 0; i < modules.size(); i++) {
            conditionBits[i] = (ConditionBit) modules.get(i);
        }
