...
public class EditModulesController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        EventHandler validityFilter = new EventHandler<InputEvent>() {
            public void handle(InputEvent event) {
                try{
                    ((ModuleController)activeTable).checkValidity(activeTable.getItems());
    ...
    public void onDeleteButtonClick(ActionEvent e){
        ...
        if ((seletedSet instanceof Register) ||
            ...
            if (cBitsThatUseIt.size() > 0) {
                ...
                Dialogs.showErrorDialog(
                        (Stage) tables.getScene().getWindow(),
                        message, "Deletion Error", "Edit Modules");
        ...
        Module oldModule =
                getCurrentController().getCurrentFromClone(seletedSet);
        if (oldModule != null) {
            ...
            if (microsThatUseIt.size() > 0) {
                ...
                Dialogs.DialogResponse response = Dialogs.showConfirmDialog(
                        (Stage) tables.getScene().getWindow(),
                        message, "Confirm Deletion", "Edit Modules");
                if(response == Dialogs.DialogResponse.CANCEL ||
                        response == Dialogs.DialogResponse.NO ||
                        response == Dialogs.DialogResponse.CLOSED)
                    return; //don't delete anything
            }
        }
     ...
     */
    public ModuleController getCurrentController(){
