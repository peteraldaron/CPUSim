...
public class PreferencesController implements Initializable {
	...
	private void saveFontTab() {
		...
		if (!tableFD.background.equals(registerBackground.getValue().toString()) ){
			Dialogs.showWarningDialog((Stage) applyButton.getScene().getWindow(),
					"The change of background color in register and RAM tables " +
					...
					for (Node label : keyBindingsPane.getChildren()){
						if (((Label)label).getText().equals(kbString)
							...
							Dialogs.DialogResponse response = Dialogs.showConfirmDialog(
									((Stage)keyBindingsPane.getScene().getWindow()),
									"The key binding of the menu item '"+conflictMenuItem
									+"' already has the key binding '"+kbString+"'.  Would you"
									+ "like to reassign the key binding of '"+
									conflictMenuItem+"'?", "Key Binding Conflict");
							...
							else if (response == Dialogs.DialogResponse.CANCEL){
								return;
							}
							else{
								keyBindings.set(keyBindingsPane.getChildren().indexOf(label), "          ");
		...
		keyBindingsPane.getScene().setOnKeyPressed(new EventHandler<KeyEvent>(){
			...
			public void handle(KeyEvent t) {
				...
				if (kbString.equals("Ctrl-X") || kbString.equals("Ctrl-V") || kbString.equals("Ctrl-A")
					...
					Dialogs.showInformationDialog((Stage)keyBindingsPane.getScene().getWindow(), 
							"The key binding "+kbString+" cannot be assigned"
									+ " to a menu item", "Reserved Key Binding");
					return;
