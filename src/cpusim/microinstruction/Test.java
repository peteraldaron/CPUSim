...
public class Test extends Microinstruction {
     ...
     * @param start an integer indicting the leftmost bit to be tested.
     ...
     */
    public Object clone(){
        return new Test(getName(),getRegister(),getStart(),getNumBits(),
    ...
    public void execute()
    {
        ...
        long signedSegment =
                (registerValue << (64 - width + start.get())) >> (64 - numBits.get());
        ...
        if (comparison.equals("EQ")) {
            if (signedSegment == value.get() || unsignedSegment == value.get())
                controlUnit.incrementMicroIndex(omission.get());
        }
            ...
            Assert.That(comparison.equals("GE"), "Illegal comparison in " +
    ...
    {
        Assert.That(oldMicro instanceof Test,
        ...
        newTest.controlUnit = controlUnit;
    }
